...
void StoreBrowserWindow(HWND hwnd, BrowserWindow* browser) {
    ...
    if (it == g_browserWindows.end()) {
        ...
        PHP_DESKTOP_LOG_WARNING << "StoreBrowserWindow() failed: already stored";
    }
}
void RemoveBrowserWindow(HWND hwnd) {
    ...
    if (it != g_browserWindows.end()) {
        ...
        PHP_DESKTOP_LOG_WARNING << "RemoveBrowserWindow() failed: not found";
    }
}
            ...
            fullscreen_(nullptr)
{
    ...
    if (IsPopup()) {
        PHP_DESKTOP_LOG_DEBUG << "BrowserWindow::BrowserWindow() created for Popup";
    } else {
...
void BrowserWindow::SetCefBrowser(CefRefPtr<CefBrowser> cefBrowser) {
    ...
    _ASSERT(!cefBrowser_);
    if (cefBrowser_) {
        PHP_DESKTOP_LOG_ERROR << "BrowserWindow::SetCefBrowser() called, "
...
bool BrowserWindow::CreateBrowserControl(const wchar_t* navigateUrl) {
    PHP_DESKTOP_LOG_DEBUG << "BrowserWindow::CreateBrowserControl()";
    ...
    if (!b) {
        PHP_DESKTOP_LOG_ERROR << "GetWindowRect() failed in "
                     "BrowserWindow::CreateBrowserControl()";
    }

    // Information used when creating the native window.
    CefWindowInfo window_info;
    CefRect cefRect = CefRect(rect.left, rect.top, (rect.right - rect.left), (rect.bottom - rect.top));
    window_info.SetAsChild(windowHandle_, cefRect);
...
void BrowserWindow::OnSize() {
    if (cefBrowser_) {
        ...
        PHP_DESKTOP_LOG_DEBUG << "BrowserWindow::OnSize(): "
...
void BrowserWindow::SetIconFromSettings() {
    ...
    if (iconPath && iconPath[0] != 0) {
        ...
        if (smallIcon) {
            ...
            PHP_DESKTOP_LOG_WARNING << "LoadIcon(IDR_MAINWINDOW) failed "
